...
namespace WinRTXamlToolkit.Controls.DataVisualization.Charting
{
    ...
    public abstract class DataPointSingleSeriesWithAxes : DataPointSeriesWithAxes, IRequireGlobalSeriesIndex
    {
        ...
        {
            //if (newDataPoints.Count == 0 && oldDataPoints.Count == 0)
            //    return;
        ...
        private void DispensedResourcesChanging()
        {
            ...
            CreateLegendItemDataPoint();
            if (DataPointStyle != null || Resources.ContainsKey(DataPointStyleName))
                ActualDataPointStyle = DataPointStyle ?? (Resources[DataPointStyleName] as Style);
            if (LegendItemStyle != null || Resources.ContainsKey(LegendItemStyleName))
